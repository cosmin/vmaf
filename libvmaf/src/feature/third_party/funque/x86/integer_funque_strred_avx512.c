...
#include "integer_funque_strred_avx512.h"
                                     ...
                                     int width, int height)
{
    ...
    for(i = 0; i < height; i++) {
        for(j = 0; j <= width - 32; j += 16) {
            ...
            __m512i dist_prev_src_v =
                _mm512_loadu_si512((__m512i *) (dist_prev_src + j + i * width));

            __m512i ref_dst_final = _mm512_sub_epi16(ref_src_v, ref_prev_src_v);
            __m512i dist_dst_final = _mm512_sub_epi16(dist_src_v, dist_prev_src_v);
...
int integer_compute_srred_funque_avx512(const struct i_dwt2buffers *ref,
                                      ...
                                      double sigma_nsq_t, uint8_t check_enable_spatial_csf)
{
    ...
    for(subband = 1; subband < total_subbands; subband++) {
        ...
        spat_values[subband] = integer_rred_entropies_and_scales_avx512(
