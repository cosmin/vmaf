                ...
                unsigned bpc, unsigned w, unsigned h)
{
    ...
    if(s->enable_resize)
    {
        ...
        s->res_ref_pic.data[0] = aligned_malloc(s->float_stride * h * bitdepth_factor, 32);
        if (!s->res_ref_pic.data[0])
            goto fail;
        s->res_dist_pic.data[0] = aligned_malloc(s->float_stride * h * bitdepth_factor, 32);
                   ...
                   unsigned index, VmafFeatureCollector *feature_collector)
{
    ...
    if(s->enable_resize)
    {
        ...
        if (ref_pic->bpc == 8)
        {
            resize((unsigned char *)ref_pic->data[0], (unsigned char *)res_ref_pic->data[0], ref_pic->w[0], ref_pic->h[0], res_ref_pic->w[0], res_ref_pic->h[0]);
            resize((unsigned char *)dist_pic->data[0], (unsigned char *)res_dist_pic->data[0], dist_pic->w[0], dist_pic->h[0], res_dist_pic->w[0], res_dist_pic->h[0]);
        }
        else
        {
