...
float strred_horz_integralsum_spatial_csf(
                                      ...
                                      int32_t pending_div_fac)
{
...

    {
        ...
        add_y = (uint64_t) (var_y + const_val);
        fscale_x =
            ((log2(add_x) * TWO_POWER_Q_FACTOR) - sub_val) / (TWO_POWER_Q_FACTOR * LOGE_BASE2);
        fscale_y =
            ((log2(add_y) * TWO_POWER_Q_FACTOR) - sub_val) / (TWO_POWER_Q_FACTOR * LOGE_BASE2);
#endif
        entropy_x = entropy_x - sub_val;
        entropy_y = entropy_y - sub_val;
        // scale_x = scale_x - sub_val;
...
float integer_rred_entropies_and_scales(const dwt2_dtype *x_t, const dwt2_dtype *y_t, size_t width,
...
void integer_subract_subbands(const dwt2_dtype *ref_src, const dwt2_dtype *ref_prev_src,
                                    ...
                                    double sigma_nsq_t, uint8_t check_enable_spatial_csf)
{
    ...
    for(subband = 1; subband < total_subbands; subband++) {
        ...
        if(prev_ref != NULL && prev_dist != NULL) {
            ...
            integer_subract_subbands(ref->bands[subband], prev_ref->bands[subband], ref_temporal,
