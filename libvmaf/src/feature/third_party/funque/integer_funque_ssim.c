...
int integer_compute_ssim_funque_c(i_dwt2buffers *ref, i_dwt2buffers *dist, double *score, int max_val, float K1, float K2, int pending_div, int32_t *div_lookup)
{
    int ret = 1;
...
int integer_compute_ms_ssim_funque_c(i_dwt2buffers *ref, i_dwt2buffers *dist, MsSsimScore_int *score,
...
int integer_compute_ms_ssim_mean_scales(MsSsimScore_int *score, int n_levels)
{
    ...
    for(int i = 1; i < n_levels; i++) {
        ...
        cum_prod_mean[i] =
            cum_prod_mean[i - 1] * pow(fabs(score[i].cs_mean), int_exps[i]) * sign_cum_prod_mean;
        cum_prod_cov[i] =
            cum_prod_cov[i - 1] * pow(fabs(score[i].cs_cov), int_exps[i]) * sign_cum_prod_cov;
        cum_prod_mink3[i] =
            cum_prod_mink3[i - 1] * pow(fabs(score[i].cs_mink3), int_exps[i]) * sign_cum_prod_mink3;
    }
    ...
    for(int i = 0; i < n_levels; i++) {
        ...
        score[i].ms_ssim_mean =
            cum_prod_concat_mean[i] * pow(fabs(score[i].ssim_mean), int_exps[i]) * sign_mssim_mean;
        score[i].ms_ssim_cov =
            cum_prod_concat_cov[i] * pow(fabs(score[i].ssim_cov), int_exps[i]) * sign_mssim_cov;

        score[i].ms_ssim_mink3 = cum_prod_concat_mink3[i] *
                                 pow(fabs(score[i].ssim_mink3), int_exps[i]) * sign_mssim_mink3;
    }
